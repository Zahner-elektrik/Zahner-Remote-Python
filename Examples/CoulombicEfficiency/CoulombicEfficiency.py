...
if __name__ == "__main__":
    ...
    for i in range(cycles):
        ...
        Charge phase
        """
        ZahnerPP2x2.measureCharge(
            current=np.abs(currentsInCycles[i]), stopVoltage=2, maximumTime="4 min"
        )
        ...
        Discharge phase
        """
        ZahnerPP2x2.measureDischarge(
            current=-1 * np.abs(currentsInCycles[i]), stopVoltage=1, maximumTime="4 min"
        )
    ...
    for cycle in range(cycles):
        efficiency = (
            np.abs(chargeWhileDischarging[cycle] / chargeWhileCharging[cycle]) * 100
        )
        coulombicEfficiency.append(efficiency)
        print(
            "Current: {:.2f} A\tEfficiency: {:.2f} %".format(
                currentsInCycles[cycle], efficiency
            )
        )
    ...
    for cycle in range(cycles):
        chargeAxis.plot(
            timeChargeCycleData[cycle],
            voltageChargeCycleData[cycle],
            label="{:.1f} A current".format(np.abs(currentsInCycles[cycle])),
        )
        dischargeAxis.plot(
            timeDischargeCycleData[cycle],
            voltageDischargeCycleData[cycle],
            label="{:.1f} A current".format(np.abs(currentsInCycles[cycle])),
        )
    ...
    cellText = [
        ["{:.1f} A".format(current) for current in currentsInCycles],
        ["{:.2f} %".format(coulombic) for coulombic in coulombicEfficiency],
    ]
